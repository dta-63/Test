<?php
/**
 * Plugin-side storage of products currently present in KAPYO carts.
 * The table is filtered against the Woo product hooks so we only push
 * webhooks for products KAPYO actually cares about.
 */

if (!defined('ABSPATH')) { exit; }

final class Kapyo_Storage {

    public static function table(): string {
        global $wpdb;
        return $wpdb->prefix . 'kapyo_active_products';
    }
     ...
     * Kapyo's periodic reconciliation to recover from drift (lost updates,
     * shop downtime, etc.).
     */
