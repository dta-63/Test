...
def _apply_filters(q, site: str, start: datetime | None, end: datetime | None):
    if site and site != "all":
        q = q.filter(PimpOrder.site_id == site)
    if start is not None:
        q = q.filter(PimpOrder.created_at >= start)
    if end is not None:
        q = q.filter(PimpOrder.created_at < end)
    return q
...
def dashboard(
    ...
    q = _apply_filters(db.query(PimpOrder), site, start, end)
    orders = q.all()
    ...
    if start is not None and end is not None:
        ...
        prev_q = _apply_filters(db.query(PimpOrder), site, ps, pe)
    ...
    top_q = (
        top_q.group_by(PimpOrderItem.product_id, PimpOrderItem.product_name, PimpOrder.site_id)
    ...
    recent_q = _apply_filters(db.query(PimpOrder), site, start, end).order_by(PimpOrder.created_at.desc()).limit(10)
    recent_orders = [
        RecentOrder(
            id=o.id, site_id=o.site_id, woo_order_number=o.woo_order_number,
            ...
            created_at=o.created_at, admin_url=_admin_url(o.site_id, o.woo_order_id),
        )
...
def list_orders(
    ...
    start, end = _period_window(period)
    base = _apply_filters(db.query(PimpOrder), site, start, end)
    if status_filter:
        base = base.filter(PimpOrder.status == status_filter)
    if q:
        like = f"%{q.strip()}%"
        base = base.filter(
            or_(
                PimpOrder.woo_order_number.ilike(like),
                PimpOrder.customer_email.ilike(like),
                PimpOrder.customer_first_name.ilike(like),
                PimpOrder.customer_last_name.ilike(like),
            ),
        )

    total = base.count()
    rows = base.order_by(PimpOrder.created_at.desc()).limit(limit).offset(offset).all()
    items = [
        RecentOrder(
            id=o.id, site_id=o.site_id, woo_order_number=o.woo_order_number,
            customer_email=o.customer_email,
...
def export_orders_csv(
    ...
    rows = base.order_by(PimpOrder.created_at.desc()).all()
    ...
    headers = {"Content-Disposition": f'attachment; filename="pimp-orders-{suffix}.csv"'}
...
def order_detail(
    ...
    o = db.query(PimpOrder).filter(PimpOrder.id == order_id).one_or_none()
    if not o:
