...
import { B2bService, OrderDetail } from './b2b.service';
...
@Component({
  ...
  template: `
    ...
    <div class="dialog" role="dialog" aria-modal="true">
      <header>
        <h2 *ngIf="order">Commande #{{ order.woo_order_number }}</h2>
  ...
  styles: [`
    ...
    header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; border-bottom: 1px solid var(--pimp-border); }
    ...
    .muted { color: var(--pimp-muted); font-size: 13px; }
    dl { display: grid; grid-template-columns: max-content 1fr; gap: 6px 16px; margin: 0; font-size: 14px; }
    dt { color: var(--pimp-muted); }
    dd { margin: 0; }
    h3 { margin: 0; font-size: 14px; text-transform: uppercase; color: var(--pimp-muted); letter-spacing: 0.05em; }
    table { width: 100%; border-collapse: collapse; }
    th, td { text-align: left; padding: 8px 6px; border-bottom: 1px solid var(--pimp-border); font-size: 13px; vertical-align: top; }
    th { color: var(--pimp-muted); font-weight: 600; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; }
    ...
    footer { display: flex; justify-content: flex-end; gap: 8px; padding: 12px 20px; border-top: 1px solid var(--pimp-border); }
