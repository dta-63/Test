...
@Component({
    ...
    <ng-container *ngIf="auth.isAuthenticated$ | async; else notLogged">
      ...
      <ng-container *ngIf="account$ | async as account">
        ...
        <section class="card danger">
          <h2>Zone sensible</h2>
          <p class="muted">
            Supprime vos données Kapyo (panier + profil local). Vous pourrez
            vous reconnecter avec le même compte Auth0, un nouveau profil
            Kapyo sera recréé automatiquement.
          </p>
          <div class="actions">
            <button class="btn-ghost" (click)="logout()">Se déconnecter</button>
            <button class="btn-danger" (click)="onDelete()">Supprimer mon compte Kapyo</button>
          </div>
        </section>
      </ng-container>
  ...
  styles: [`
    ...
    .card {
      ...
      border: 1px solid var(--kapyo-border); margin-bottom: 16px;
    }
    ...
    .avatar {
      ...
      object-fit: cover; border: 2px solid var(--kapyo-border);
    }
    .profile-body h2, .card h2 { margin: 0 0 8px; font-size: 18px; }
    .muted { color: var(--kapyo-muted); margin: 0 0 12px; }
    ...
    .stat {
      background: var(--kapyo-bg); padding: 16px; border-radius: 8px;
      display: flex; flex-direction: column; gap: 4px;
    }
    .stat-label { font-size: 12px; color: var(--kapyo-muted); text-transform: uppercase; letter-spacing: 0.05em; }
    .stat-value { font-size: 24px; font-weight: 700; color: var(--kapyo-primary); }
    .by-site { margin-top: 20px; }
    .by-site h3 { margin: 0 0 8px; font-size: 14px; text-transform: uppercase; color: var(--kapyo-muted); letter-spacing: 0.05em; }
...
export class AccountComponent {
  ...
  onDelete(): void {
    const ok = window.confirm(
      'Supprimer toutes vos données Kapyo (panier, profil local) ? Cette action est irréversible.',
    );
    if (!ok) return;
