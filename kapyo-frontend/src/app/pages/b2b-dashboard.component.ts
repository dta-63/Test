...
import { B2bOrderDetailComponent } from '@components/b2b-order-detail.component';
import {
  Dashboard, DashboardFilters, OrdersFilters, OrdersPage, Period, RecentOrder, SiteFilter,
...
@Component({
  ...
  styles: [`
    ...
    .seg { display: inline-flex; background: white; border: 1px solid var(--kapyo-border); border-radius: 8px; overflow: hidden; }
    .seg button { background: transparent; padding: 6px 12px; font-size: 13px; color: var(--kapyo-text); border-radius: 0; border-right: 1px solid var(--kapyo-border); }
    .seg button:last-child { border-right: none; }
    .seg button.active { background: var(--kapyo-primary); color: white; }
    .kpis { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 12px; margin-bottom: 20px; }
    .kpi { background: white; padding: 16px; border-radius: 12px; border: 1px solid var(--kapyo-border); display: flex; flex-direction: column; gap: 4px; }
    .kpi .label { font-size: 12px; color: var(--kapyo-muted); text-transform: uppercase; letter-spacing: 0.05em; }
    .kpi .value { font-size: 26px; font-weight: 700; color: var(--kapyo-primary); }
    .kpi .sub { font-size: 12px; color: var(--kapyo-muted); }
    ...
    .delta .muted { color: var(--kapyo-muted); font-weight: 400; }
    .grid { display: grid; gap: 16px; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); margin-bottom: 16px; }
    .card { background: white; padding: 16px; border-radius: 12px; border: 1px solid var(--kapyo-border); }
    .card header { display: flex; align-items: baseline; justify-content: space-between; margin-bottom: 12px; }
    .card h2 { margin: 0; font-size: 14px; text-transform: uppercase; color: var(--kapyo-muted); letter-spacing: 0.05em; }
    .muted { color: var(--kapyo-muted); font-size: 12px; }
    .center { text-align: center; padding: 20px !important; }
    .chart-card .sparkline { width: 100%; height: 140px; display: block; }
    .chart-card .axis { display: flex; justify-content: space-between; font-size: 11px; color: var(--kapyo-muted); margin-top: 6px; }
    ...
    .bar-track { background: var(--kapyo-bg); border-radius: 4px; height: 8px; overflow: hidden; margin-top: 4px; }
    .bar-fill { height: 100%; }
    .bar-fill[data-site="site-a"] { background: var(--kapyo-shop-a); }
    .bar-fill[data-site="site-b"] { background: var(--kapyo-shop-b); }
    ...
    .status-pill { padding: 2px 10px; border-radius: 999px; font-size: 11px; background: #f3f4f6; color: var(--kapyo-text); }
    ...
    .rank { width: 24px; height: 24px; border-radius: 50%; background: var(--kapyo-bg); color: var(--kapyo-primary); font-weight: 700; font-size: 12px; display: inline-flex; align-items: center; justify-content: center; }
    ...
    .recent-tools input {
      padding: 8px 10px; border: 1px solid var(--kapyo-border); border-radius: 6px;
      font-size: 13px; min-width: 240px;
    }
    .recent table { width: 100%; border-collapse: collapse; }
    .recent th, .recent td { padding: 10px 8px; border-bottom: 1px solid var(--kapyo-border); text-align: left; font-size: 13px; vertical-align: top; }
    .recent th { color: var(--kapyo-muted); font-weight: 600; font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; }
    .recent td a { color: var(--kapyo-primary); }
    .row-clickable { cursor: pointer; }
    .row-clickable:hover { background: var(--kapyo-bg); }
    .pager { display: flex; justify-content: space-between; align-items: center; margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--kapyo-border); }
  ...
  exportCsv(): void {
    ...
    this.api.downloadCsv(filters).subscribe({
      next: (blob) => {
        ...
        a.href = url;
        a.download = `kapyo-orders-${this.period()}-${this.site()}.csv`;
